...
出題のある4モードはこの1つの仕組みに乗っている。ヒント段階の計算は `hintStage()` として
DOMなしで試験できる純関数に切り出してある。

`free: true` の質問は「お願いがない盤面」を表す。文字ばたけと
「たぬきと あそぶ」4種がこれで、どれを触っても正解として扱われる。

**1問終わってから次の問題が出るまでの間、`state.quest` は `null`**。
この空白がないと、答え終わった問題に対して次のタップが採点されてしまう
（実際にブラウザ試験で見つけた不具合）。

### 3.1 「たぬきと あそぶ」は Quest に採点させない

4種の遊びモードは `state.quest` を**誘導のためだけ**に持つ（`startFreePlay()`）。
タップは `resolveTap()` を通らず、各要素が自分のハンドラで直接反応する。理由は2つ。

1. **`state.busy` を立てないため。** 出題モードでは演出中のタップを止める必要があるが
   （§3の空白と同じ理由）、遊びモードではその防御が「叩いても鳴らない楽器」になる。
2. **待ち時間を作らないため。** 出題モードは `STEP_PACING` で次の問題まで間を置く。
   楽器やしゃぼん玉に同じ間を置いたら、それは反応の遅い玩具でしかない。

進行は `playStep()` が数える。**すでに満杯なら -1 を返す**ので、指が2本同時に着地しても
棚があふれず、完成演出が二重に予約されることもない。

盤面が満杯になった瞬間の一発芸は `state.roundFinale` に登録する
（おんがくたい＝全員が順に鳴る、ごはん＝満腹の腹鼓）。`clearRuntime()` が消す。
...
### 5.1 隠れ場所SVG（いないいないばあ）

同じ 200×200 の作りで、`HIDEOUTS` に6種。生育地と違うのは**出演者の隠し方**。

- `coverTop`: 開口部の高さ（％）。ここより上が「窓」、ここより下に容器を描く。
- `guestScale` / `lip`: 出演者の大きさと、開口部にどれだけ沈めるか。

出演者は `overflow:hidden` の窓の中にいて、隠れている状態では窓の下端より下へ
translate されている。つまり**隠れているあいだは本当に描画されていない**。
透明度で隠すと影やアンチエイリアスが残り、驚きが成立しない。

`lip` は大きめ（17〜22％）に取ってある。スプライトには透明な余白があるので、
小さい `lip` だと出演者が容器の縁の上に浮いて見える。

切り株とかごの縁は、**手前側の弧だけ**を `front` に置いている。楕円を丸ごと前面に置くと、
中に立っている出演者の胸を線が横切る。
...
| ブラウザ試験 | `node scripts/browser-smoke.mjs` | 実ブラウザで全10モードを**各モードの本来の操作で**完走、コンソールエラー、はみ出し、タップ対象の大きさ、**誘導が指す先＝触るべき要素**かつ**指の動き＝必要な操作**、誤選択で進行が壊れないこと、吹き出しの中身が吹き出しからはみ出さないこと、**同形のはずの盤面のマスが同じ大きさであること**、古いビルドでないこと |
